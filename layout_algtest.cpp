...
tile_to_mma_shape(LayoutAtom const& atom, MMATileShape const& mma_tile_shape, ModeOrder const& order = {})
{
  ...
  std::cout<< "mma_tile_shape: " << mma_tile_shape << std::endl;
  auto mn_shape = cute::tuple_cat(zip(shape<0>(mma_tile_shape), take<1,3>(mma_tile_shape)), take<3,R>(mma_tile_shape));
  std::cout<< "mn_shape: " << mn_shape <<std::endl;
  auto mn_tiled = tile_to_shape(atom, mn_shape, order);                      // (BLK_M,BLK_N,...)
  std::cout << "tile_to_shape: " << atom << " and " << mn_shape << std::endl <<"\t-->";
  std::cout << mn_tiled << std::endl;

  std::cout << std::endl << "tiled_divide "<<mn_tiled
    <<" to "<< product_each(shape<0>(mma_tile_shape)) <<std::endl <<"\t-->";
...
int main() {
  ...
  // Stack verticle
  // using Layout_normal = Layout<Shape<_128,_128>,Stride<_1,_128>>;
  // using Layout_normal = Layout<Shape<_128,_256>,Stride<_256,_1>>;
